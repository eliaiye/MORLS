    ...
    def rollout(self, env, timesteps, gamma, lamb): # timesteps << episode length
        ...
        with torch.set_grad_enabled(False):
            ...
            for i in range(timesteps):
                ...
                action, logprob, _, value = self.forward(observation = obs) # action: tensor.size([1, act_dim]), logprob: tensor.size([1]), value: tensor.size([1])
                ...
                #obs, reward, done, _ = env.step(action.numpy().astype(float))
            ...
            _, __, ___, last_value = self.forward(obs)
...
#                         adv_trajectory[t]=delta_trajectory[t]+gamma*lamb*adv_trajectory[t+1]
    ...
    #                 adv_trajectory[t]=delta_trajectory[t]+gamma*lamb*adv_trajectory[t+1] #recursion
            ...
            return_trajectory = adv_trajectory + value_trajectory
        
        return(obs_trajectory, action_trajectory, logprob_trajectory, adv_trajectory, value_trajectory, return_trajectory)
    
    def loss(self, epsilon, c1, c2, obs_traj, action_traj, logprob_traj, adv_traj, value_traj, return_traj, current_logprob, current_entropy, current_value):
        # entropy loss
        entropy_loss = -current_entropy.mean()
        # value loss
        value_loss = F.mse_loss(return_traj, current_value)
        ...
        total_loss = PGloss + c1*value_loss + c2*entropy_loss
        
        return total_loss # torch.size([])
    
    def train_step(self, epsilon, c1, c2, obs_traj, action_traj, logprob_traj, adv_traj, value_traj, return_traj):
        
        #Question: adv needs normalization?
        
        self.policy.train() # module train mode
        with torch.set_grad_enabled(True):
            action, logprob, entropy, value = self.forward(obs_traj, action=action_traj)
            
            total_loss = self.loss(epsilon, c1, c2, obs_traj, action_traj, logprob_traj, adv_traj, value_traj, return_traj, logprob, entropy, value)
    ...
    def train(self, env, total_timesteps, timesteps=1024, k_epoch=10, num_batch=4, n_actor=1, 
              epsilon=0.2, c1=0.5, c2=0.0, gamma = 0.99, lamb=0.95, lr = 0.0003, max_grad_norm = 0.5, optimizer = torch.optim.Adam, 
        ...
        for i in tqdm(range(n_updates)):
            ...
            for actor in range(n_actor-1):
                ...
                value_trajectory = torch.cat((value_trajectory, more_value_traj),0)
                return_trajectory = torch.cat((return_trajectory, more_return_traj),0)
        ...
        for i_episode in range(n_ep):
            ...
            for t in range(timeout):
                with torch.set_grad_enabled(False):
                    ...
                    action, _, __, ___=self.forward(observation)
    ...
    def eval(self, obs):
        with torch.set_grad_enabled(False):
            obs = torch.unsqueeze(torch.tensor(obs).float(),0)
            action, _, __, ____=self.forward(obs)
